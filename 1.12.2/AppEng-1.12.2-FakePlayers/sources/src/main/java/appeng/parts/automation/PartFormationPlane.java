...
public class PartFormationPlane extends PartAbstractFormationPlane<IAEItemStack>
{
	...
	private final MEInventoryHandler<IAEItemStack> myHandler = new MEInventoryHandler<>(this, AEApi.instance().storage().getStorageChannel(IItemStorageChannel.class));
	...
	public IAEItemStack injectItems(final IAEItemStack input, final Actionable type, final IActionSource src)
	{
		...
		if (w.getBlockState(tePos).getBlock().isReplaceable(w, tePos))
			if (placeBlock == YesNo.YES && (i instanceof ItemBlock || i instanceof ItemBlockSpecial || i instanceof IPlantable || i instanceof ItemSkull || i instanceof ItemFirework || i instanceof IPartItem || i == Item.getItemFromBlock(Blocks.REEDS)))
			{
				...
				if (type == Actionable.MODULATE)
				{
					...
					if (i instanceof IPlantable || i instanceof ItemSkull || i == Item.getItemFromBlock(Blocks.REEDS))
					{
						...
						if (side.xOffset == 0 && side.zOffset == 0)
							Worked = i.onItemUse(player, w, tePos.offset(side.getFacing()), hand, interactSide, side.xOffset, side.yOffset, side.zOffset) == EnumActionResult.SUCCESS;

						if (!Worked && side.xOffset == 0 && side.zOffset == 0)
							Worked = i.onItemUse(player, w, tePos.offset(interactSide), hand, side.getFacing(), side.xOffset, side.yOffset, side.zOffset) == EnumActionResult.SUCCESS;

						if (!Worked && side.yOffset == 0)
							Worked = i.onItemUse(player, w, tePos.offset(EnumFacing.DOWN), hand, EnumFacing.UP, side.xOffset, side.yOffset, side.zOffset) == EnumActionResult.SUCCESS;

						if (!Worked)
							i.onItemUse(player, w, tePos, hand, interactSide, side.xOffset, side.yOffset, side.zOffset);
