...
public class BlockForestryFluid extends BlockFluidClassic implements IItemModelRegister
{
	...
	public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand)
	{
		...
		if (this.blockMaterial == Material.WATER)
		{
			...
			if (i > 0 && i < 8)
			{
				if (this.getFluid().getViscosity(worldIn, pos) < 5000 && rand.nextInt(64) == 0)
		...
		if (rand.nextInt(10) == 0 && worldIn.getBlockState(pos.down()).isSideSolid(worldIn, pos.down(), EnumFacing.DOWN))
		{
			...
			if (!material.blocksMovement() && !material.isLiquid())
			{
				...
				Particle fx = new ParticleColoredDripParticle(worldIn, px, py, pz, this.color.getRed() / 255f, this.color.getGreen() / 255f, this.color.getBlue() / 255f);
	...
	public boolean canDisplace(IBlockAccess world, BlockPos pos)
	{
		...
		return !blockState.getMaterial().isLiquid() && super.canDisplace(world, pos);
	}
	...
	{
		IBlockState blockState = world.getBlockState(pos);
		return !blockState.getMaterial().isLiquid() && super.displaceIfPossible(world, pos);
	}
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random rand)
	{
		...
		if (this.getMaterial(state) == Material.LAVA)
		{
		...
		int flammability = this.getFlammability(world, pos, null);
		if (flammability > 0)
		{
