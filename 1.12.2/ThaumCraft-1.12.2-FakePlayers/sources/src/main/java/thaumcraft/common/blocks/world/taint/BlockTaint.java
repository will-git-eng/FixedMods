...
public class BlockTaint extends BlockTC implements ITaintBlock
{
	...
	public void updateTick(World world, BlockPos pos, IBlockState state, Random random)
	{
		if (!world.isRemote)
		{
			...
			if (state.getBlock() == BlocksTC.taintCrust)
			{
				...
				if ((double) world.rand.nextFloat() < 0.2D && world.isAnyPlayerWithinRangeAt((double) ((float) pos.getX() + 0.5F), (double) ((float) pos.getY() + 0.5F), (double) ((float) pos.getZ() + 0.5F), 32.0D) && EntityUtils.getEntitiesInRange(world, pos, null, EntityTaintSwarm.class, 32.0D).isEmpty())
				{
	...
	public void onEntityWalk(World world, BlockPos pos, Entity entity)
	{
		...
		if (EventConfig.potionFluxTaint && !world.isRemote && entity instanceof EntityLivingBase && !((EntityLivingBase) entity).isEntityUndead() && world.rand.nextInt(250) == 0)
	...
	public static boolean canFallBelow(World world, BlockPos pos)
	{
		...
		if (l.isAir(bs, world, pos))
			return true;
		if (l == BlocksTC.fluxGoo && bs.getValue(BlockFluidFinite.LEVEL) >= 4)
			return false;
		if (l != Blocks.FIRE && l != BlocksTC.taintFibre)
		{
