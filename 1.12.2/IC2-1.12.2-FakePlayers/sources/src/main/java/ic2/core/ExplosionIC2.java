...
public class ExplosionIC2 extends Explosion
{
	...
	public ExplosionIC2(World world, Entity entity, double x, double y, double z, float power1, float drop, ExplosionIC2.Type type1, EntityLivingBase igniter, int radiationRange1)
	{
		...
		if (!this.fake.setRealPlayer(entity))
			this.fake.setRealPlayer(this.igniter);
				...
				if (this.isNuclear() && this.radiationRange >= 1)
					for (EntityLiving entity : this.worldObj.getEntitiesWithinAABB(EntityLiving.class, new AxisAlignedBB(this.explosionX - this.radiationRange, this.explosionY - this.radiationRange, this.explosionZ - this.radiationRange, this.explosionX + this.radiationRange, this.explosionY + this.radiationRange, this.explosionZ + this.radiationRange)))
					{
						if (!ItemArmorHazmat.hasCompleteHazmat(entity))
						{
							...
							if (EventConfig.explosionEvent && (hungerLength >= 0 || poisonLength >= 0) && this.fake.cantAttack(entity))
								continue;
				...
				for (int y = 0; y < this.destroyedBlockPositions.length; ++y)
				{
					...
					if (bitSet != null)
						for (int index = -2; (index = nextSetIndex(index + 2, bitSet, 2)) != -1; )
						{
							...
							if (EventConfig.explosionEvent && this.fake.cantBreak(tmpPos))
								continue;
							...
							if (doDrops && block.canDropFromExplosion(this) && getAtIndex(index, bitSet, 2) == 1)
								for (ItemStack stack : StackUtil.getDrops(this.worldObj, tmpPos, state, block, 0))
								{
									if (rng.nextFloat() <= this.explosionDropRate)
									{
										...
										Map<ItemComparableItemStack, DropData> map = blocksToDrop.computeIfAbsent(xZposition, k -> new HashMap<>());
	...
	private void shootRay(double x, double y, double z, double phi, double theta, double power1, boolean killEntities, MutableBlockPos tmpPos)
	{
		...
		float sinTheta = MathHelper.sin((float) theta);
		double deltaX = sinTheta * MathHelper.cos((float) phi);
		double deltaY = MathHelper.cos((float) theta);
		double deltaZ = sinTheta * MathHelper.sin((float) phi);
