...
public final class ConfigUtils
{
	...
	{
		Configuration cfg = new Configuration(new File(reflectionmedic.cfgDir, cfgName + ".cfg"));
		cfg.load();
		return cfg;
	}
	...
	public static void readConfig(@Nonnull Class<?> configClass, @Nonnull String configName, boolean reload)
	{
		...
		try
		{
			for (Field field : configClass.getDeclaredFields())
			{
				...
				{
					reflectionmedic.LOGGER.error("Failed reading property {} in config {}", field, cfg.getConfigFile().getName(), throwable);
				}
			}
		}
		catch (Throwable throwable)
		{
			reflectionmedic.LOGGER.error("Failed reading config {}", cfg.getConfigFile().getName(), throwable);
		}
		cfg.save();
	}
