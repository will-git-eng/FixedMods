...
public final class CompendiumEvents implements IExtendedPropertyInitializer<PlayerCompendiumData>
{
	...
	public static KnowledgeObservation getObservation(EntityPlayer player)
	{
		...
		if (distBlock < distEntity && mopBlock != null)
		{
			...
			if (tracedEntity instanceof EntityLiving)
				observationReuse.setMob(KnowledgeObject.<ObjectMob>getObject(tracedEntity));
			else if (tracedEntity instanceof EntityItem)
			{
				ItemStack is = ((EntityItem) tracedEntity).getEntityItem();

				if (is.getItem() instanceof ItemBlock)
					observationReuse.setBlock(getBlockObject(is));
				else
					observationReuse.setItem(KnowledgeObject.<ObjectItem>getObject(is.getItem()));
			}

		return observationReuse;
	}

	private final TObjectByteHashMap<UUID> playerTickLimiter = new TObjectByteHashMap<UUID>();
	...
	{
		this.playerTickLimiter.remove(e.player.getGameProfile().getId());
	}

	@SubscribeEvent
	public void onPlayerTick(PlayerTickEvent e)
	{
		if (e.phase != Phase.START || e.player.worldObj.isRemote)
			return;

		EntityPlayer player = e.player;

		if (this.playerTickLimiter.adjustOrPutValue(player.getGameProfile().getId(), byteOne, byteOne) >= 7)
		{
			Stopwatch.timeAverage("CompendiumEvents - look tracing", 100);

			this.playerTickLimiter.put(player.getGameProfile().getId(), byteZero);
	...
	{
		if (e.player != null && !e.player.worldObj.isRemote)
			this.discoverItemStack(e.player, e.pickedUp.getEntityItem());
	}
