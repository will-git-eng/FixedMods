...
public class BlockEnergyCluster extends BlockContainer
{
	...
	public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entity, ItemStack stack)
	{
		...
		TileEntity tile;
		if (entity instanceof EntityPlayer && (tile = world.getTileEntity(x, y, z)) instanceof TileEntityEnergyCluster)
			((TileEntityEnergyCluster) tile).fake.setProfile(((EntityPlayer) entity).getGameProfile());
	}
	...
	public boolean addHitEffects(World world, MovingObjectPosition target, EffectRenderer effectRenderer)
	{
		for (int a = 0; a < 4; a++)
		{
			effectRenderer.addEffect(new EntityEnergyClusterFX(world, target.blockX + 0.5D, target.blockY + 0.5D, target.blockZ + 0.5D, 0D, 0D, 0D, 0D, 0D, 0D));
		}
		return true;
	}
	...
	public boolean addDestroyEffects(World world, int x, int y, int z, int meta, EffectRenderer effectRenderer)
	{
		for (int a = 0; a < 4; a++)
		{
			effectRenderer.addEffect(new EntityEnergyClusterFX(world, x + 0.5D, y + 0.5D, z + 0.5D, 0D, 0D, 0D, 0D, 0D, 0D));
		}
		return true;
	}
	...
	public static void destroyCluster(TileEntityEnergyCluster tile)
	{
		...
		boolean blockGrief = EventConfig.energyClusterBlockGrief;
		ExplosionByPlayer.newExplosion(tile.fake.get(), world, null, x + 0.5D, y + 0.5D, z + 0.5D, 2.8F + (energyMeta - 3) * 0.225F, blockGrief, blockGrief);
				...
				for (int zz = z - idist; zz <= z + idist; zz++)
				{
					for (int yy = y - idist; yy <= y + idist; yy++)
					{
						if (MathUtil.distance(xx - x, yy - y, zz - z) <= dist && world.isAirBlock(xx, yy, zz))
						{
							...
							if (tile.fake.cantBreak(xx, yy, zz))
								continue;
