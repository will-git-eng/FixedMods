...
public class BlockCoverable extends BlockContainer
{
	 ...
	 *
	 * @param dir
	 *            the rotated {@link ForgeDirection}
	 */
	 ...
	 *            the z coordinate
	 * @param itemId
	 *            the eventId, repurposed
	 * @param metadata
	 *            the event parameter, repurposed
	 ...
	 *            the z coordinate
	 * @param dropBlock
	 *            whether block {@link ItemStack} is dropped
	 */
			...
			if (TE != null)
			{
				...
				if (TE_adj.getBlockType().isSideSolid(blockAccess, x, y, z, side_adj) == TE_src.getBlockType().isSideSolid(blockAccess, x + side_adj.offsetX, y + side_adj.offsetY, z + side_adj.offsetZ, ForgeDirection.getOrientation(side)))
					if (this.shareFaces(TE_adj, TE_src, side_adj, side_src))
					{
						...
						else if (!TE_src.hasAttribute(TEBase.ATTR_COVER[6]) && block_adj.getRenderBlockPass() == 0)
							return !block_adj.isOpaqueCube();
						else if (TE_src.hasAttribute(TEBase.ATTR_COVER[6]) && block_src.isOpaqueCube() == block_adj.isOpaqueCube() && block_src.getRenderBlockPass() == block_adj.getRenderBlockPass())
							return false;
						else
							return true;
					}
