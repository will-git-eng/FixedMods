...
public class SpellBase extends ItemSpellBase
{
		...
		if (stack.hasTagCompound())
		{
			...
			if (Keyboard.isKeyDown(42))
			{
				...
				for (Affinity aff : affinityData.keySet())
					list.add(String.format("%s (%.2f%%)", new Object[] { aff.toString(), Float.valueOf(affinityData.get(aff).floatValue() * 100.0F) }));
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int par4, boolean par5)
	{
		...
		if (entity instanceof EntityPlayerSP)
		{
			...
			if (usingItem != null && usingItem.getItem() == this && SkillData.For(player).isEntryKnown(SkillTreeManager.instance.getSkillTreeEntry(SkillManager.instance.getSkill("SpellMotion"))))
			{
