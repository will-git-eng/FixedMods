...
public class AffinityHelper
{
	...
	public void onEntityLivingBase(LivingUpdateEvent event)
	{
		...
		if (ent instanceof EntityEnderman && ent.getLastAttacker() != ent.getAITarget() && ent.getAITarget() instanceof EntityPlayer)
		{
			AffinityData affinityData = AffinityData.For(ent.getAITarget());
			float enderDepth = affinityData.getAffinityDepth(Affinity.ENDER);
			if (enderDepth == 1.0F)
				ent.setRevengeTarget((EntityLivingBase) null);
		}
		...
		if (ent instanceof EntityPlayer)
		{
			...
			affinityData.accumulatedLifeRegen = (float) (affinityData.accumulatedLifeRegen + 0.025D * lifeDepth);
			if (affinityData.accumulatedLifeRegen > 1.0F)
			{
				--affinityData.accumulatedLifeRegen;
				ent.heal(1.0F);
			}
			...
			if (natureDepth == 1.0F)
			{
				if (ent.worldObj.canBlockSeeTheSky((int) ent.posX, (int) ent.posY, (int) ent.posZ) && ent.worldObj.isDaytime())
				{
					...
					if (affinityData.accumulatedHungerRegen > 1.0F)
					{
						...
						--affinityData.accumulatedHungerRegen;
					}
				}
				else
				...
				if (ent.isCollidedHorizontally)
				{
					if (!ent.isSneaking())
					{
						...
						ent.moveEntity(0.0D, movement, 0.0D);
						ent.motionY = 0.0D;
					}
					else
						ent.motionY *= 0.79999999D;

					ent.fallDistance = 0.0F;
				}
			}

			if (enderDepth >= 0.75F && affinityData.hasActivatedNightVision() && !ent.worldObj.isRemote && (!ent.isPotionActive(Potion.nightVision.id) || ent.getActivePotionEffect(Potion.nightVision).getDuration() <= 220))
			...
			if (ent.isInWater())
			{
				...
				if (earthDepth > 0.25F && ent.motionY > -0.30000001192092896D)
					ent.addVelocity(0.0D, -0.01F * earthDepth, 0.0D);

				if (waterDepth > 0.5F && (!ent.isPotionActive(BuffList.swiftSwim.id) || ent.getActivePotionEffect(BuffList.swiftSwim).getDuration() < 10))
					ent.addPotionEffect(new BuffEffectSwiftSwim(100, waterDepth > 0.75F ? 2 : 1));

				if (waterDepth > 0.4D && ent.worldObj.rand.nextInt(20) < 4)
			...
			if (ent.worldObj.isRaining() && !ent.worldObj.isRemote && ent.worldObj.getBiomeGenForCoords((int) Math.floor(ent.posX), (int) Math.floor(ent.posZ)).canSpawnLightningBolt())
			{
				...
				if (airDepth > 0.5F && airDepth < 0.85F && !ent.worldObj.isRemote && ent.worldObj.rand.nextInt(100) < 10 && !ent.isSneaking() && !ent.isPotionActive(BuffList.gravityWell) && !ent.isInsideOfMaterial(Material.water) && ent.isWet())
				{
					double velX = ent.worldObj.rand.nextDouble() - 0.5D;
					double velY = ent.worldObj.rand.nextDouble() - 0.5D;
					double velZ = ent.worldObj.rand.nextDouble() - 0.5D;
					ent.addVelocity(velX, velY, velZ);
					AMNetHandler.INSTANCE.sendVelocityAddPacket(ent.worldObj, ent, velX, velY, velZ);
				}
			}

			if (ent.isSneaking() && iceDepth >= 0.5F)
	...
	private void applyFulmintion(EntityPlayer ent, float lightningDepth)
	{
		if (!ent.worldObj.isRemote)
		{
			if (lightningDepth > 0.5F && lightningDepth <= 0.8F)
			{
				...
				if (block == Blocks.tnt && !EventUtils.cantBreak(ent, offsetX, offsetY, offsetZ))
				{
			...
			if (lightningDepth >= 0.7F && lightningDepth <= 0.95F && ent.getRNG().nextDouble() < 0.05000000074505806D)
				for (EntityCreeper creeper : (Iterable<EntityCreeper>) ent.worldObj.getEntitiesWithinAABB(EntityCreeper.class, ent.boundingBox.expand(5.0D, 5.0D, 5.0D)))
				{
		...
		{
			ExtendedProperties.For(ent).deductMana(100.0F);
			if (ent.worldObj.isRemote)
				AMCore.proxy.particleManager.BoltFromEntityToPoint(ent.worldObj, ent, ent.posX - 2.0D + ent.getRNG().nextDouble() * 4.0D, ent.posY + ent.getEyeHeight() - 2.0D + ent.getRNG().nextDouble() * 4.0D, ent.posZ - 2.0D + ent.getRNG().nextDouble() * 4.0D);
			else if (ent.getRNG().nextDouble() < 0.4000000059604645D)
	...
	private void makeIceBridge(EntityPlayer ent, float iceDepth)
	{
		...
		for (int i = 0; i < blocks.length; ++i)
		{
			...
			for (int n = -1; n <= 1; ++n)
				for (int p = -1; p <= 1; ++p)
				{
					int x = (int) current.x + n;
					int y = (int) current.y;
					int z = (int) current.z + p;

					Block placeBlock = null;

					Block block = ent.worldObj.getBlock(x, y, z);
					if (iceDepth == 1.0F && block == Blocks.lava)
						placeBlock = Blocks.obsidian;
					else if (iceDepth == 1.0F && block == Blocks.flowing_lava)
						placeBlock = Blocks.cobblestone;
					else if (block == Blocks.water)
						placeBlock = Blocks.ice;
					...
					if (placeBlock != null)
					{
						...
						if (EventUtils.cantBreak(ent, x, y, z))
							continue;
						    

						ent.worldObj.setBlock(x, y, z, placeBlock);
					}

					if (ent.worldObj.getBlock(x, y + 1, z) == Blocks.fire)
					{
						    
						if (EventUtils.cantBreak(ent, x, y + 1, z))
							continue;
						    

						ent.worldObj.setBlock(x, y + 1, z, Blocks.air);
					}
				}
		}
	...
	public void onEntityJump(LivingJumpEvent event)
	{
		...
		if (ent instanceof EntityPlayer)
		{
			...
			if (airDepth >= 0.5F)
			{
				...
				ent.addVelocity(0.0D, velocity, 0.0D);
			}
	...
	public void onEntityFall(LivingFallEvent event)
	{
		...
		if (ent instanceof EntityPlayer)
		{
			...
			float earthDepth = affinityData.getAffinityDepth(Affinity.EARTH);
			if (earthDepth > 0.25F)
				event.distance = (float) (event.distance + 1.25D * earthDepth);
	...
	public void onEntityHurt(LivingHurtEvent event)
	{
		...
		if (ent instanceof EntityPlayer)
		{
			...
			if (waterDepth > 0.9F && event.source.getSourceOfDamage() instanceof EntityEnderman)
				event.source.getSourceOfDamage().attackEntityFrom(DamageSource.drown, 2.0F);
			...
			if (event.source.getSourceOfDamage() instanceof EntityLivingBase)
			{
				float natureDepth = affinityData.getAffinityDepth(Affinity.NATURE);
				if (natureDepth == 1.0F)
					((EntityLivingBase) event.source.getSourceOfDamage()).attackEntityFrom(DamageSource.cactus, 3.0F);
				else if (natureDepth >= 0.75F)
					((EntityLivingBase) event.source.getSourceOfDamage()).attackEntityFrom(DamageSource.cactus, 2.0F);
				else if (natureDepth >= 0.5F)
					((EntityLivingBase) event.source.getSourceOfDamage()).attackEntityFrom(DamageSource.cactus, 1.0F);
	...
	{
		if (event.caster.isPotionActive(BuffList.clarity.id) && !event.caster.worldObj.isRemote)
	...
	public void onSpellCast(SpellCastingEvent.Post event)
	{
		if (event.caster instanceof EntityPlayer && event.castResult == SpellCastResult.SUCCESS)
		{
			...
			if (affinityDepth > 0.4F && event.caster.worldObj.rand.nextInt(100) < 5 && !event.caster.worldObj.isRemote)
