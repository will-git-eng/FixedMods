...
public class Telekinesis implements ISpellComponent
{
	...
	private boolean doTK_Extrapolated(ItemStack stack, World world, double impactX, double impactY, double impactZ, EntityLivingBase caster)
	{
		...
		for (Entity e : entities)
			if (e.ticksExisted >= 20)
			{
				...
				if (EventUtils.cantBreak(player, e.posX, e.posY, e.posZ))
					return false;
	...
	public void spawnParticles(World world, double x, double y, double z, EntityLivingBase caster, Entity target, Random rand, int colorModifier)
	{
		if (caster instanceof EntityPlayer)
		{
			...
			if (mop == null)
			{
				x = caster.posX + Math.cos(Math.toRadians(caster.rotationYaw + 90.0F)) * range;
				z = caster.posZ + Math.sin(Math.toRadians(caster.rotationYaw + 90.0F)) * range;
