...
public class EventHandler
{
				...
				for (Object render : RenderManager.instance.entityRenderMap.values())
				{
					if (RenderMinecart.class.isAssignableFrom(render.getClass()))
					{
						...
						if (o instanceof ModelMinecart)
						{
							...
							ObfuscationReflectionHelper.setPrivateValue(RenderMinecart.class, (RenderMinecart) render, new ModelShaderMinecart(wrapped), "field_77013_a", "modelMinecart");
						}
					}
				}
	...
	public void onEntityConstructing(EntityConstructing event)
	{
		if (event.entity instanceof EntityMinecart)
		{
			...
			{
				if (invalid.isAssignableFrom(event.entity.getClass()))
					return;
			}
		...
		if (event.phase == TickEvent.Phase.START && validateConnsNextTick && FMLCommonHandler.instance().getEffectiveSide() == Side.SERVER)
		{
			...
			for (int dim : ImmersiveNetHandler.INSTANCE.getRelevantDimensions())
			{
				...
				if (validateConnections)
				{
					...
					{
						if (!(world.getTileEntity(con.start.posX, con.start.posY, con.start.posZ) instanceof IImmersiveConnectable && world.getTileEntity(con.end.posX, con.end.posY, con.end.posZ) instanceof IImmersiveConnectable))
						{
	...
	public void onAnvilChange(AnvilUpdateEvent event)
	{
		if (event.left != null && event.left.getItem() instanceof IDrillHead && ((IDrillHead) event.left.getItem()).getHeadDamage(event.left) > 0)
		{
			if (event.right != null && event.left.getItem().getIsRepairable(event.left, event.right))
			{
				...
				int repair = Math.min(((IDrillHead) event.output.getItem()).getHeadDamage(event.output), ((IDrillHead) event.output.getItem()).getMaximumHeadDamage(event.output) / 4);
				int cost = 0;
				for (; repair > 0 && cost < event.right.stackSize; ++cost)
				{
					...
					repair = Math.min(((IDrillHead) event.output.getItem()).getHeadDamage(event.output), ((IDrillHead) event.output.getItem()).getMaximumHeadDamage(event.output) / 4);
				}
