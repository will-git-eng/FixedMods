...
public class EntityMiningLaser extends Entity implements IThrowableEntity
{
	...
	public static Set<Block> unmineableBlocks = new HashSet(Arrays.asList(new Block[] { Blocks.brick_block, Blocks.obsidian, Blocks.lava, Blocks.flowing_lava, Blocks.water, Blocks.flowing_water, Blocks.bedrock }));
			...
			if (movingobjectposition != null && IC2.platform.isSimulating())
			{
				...
				if (movingobjectposition.entityHit != null)
				{
					...
					if (this.takeDataFromEvent(tEvent))
					{
						...
						if (damage > 0)
							...
							if (this.fake.cantDamage(tEvent.hitentity))
							{
								this.setDead();
								return;
				...
				else
				{
					...
					if (this.takeDataFromEvent(tEvent))
					{
						...
						int meta = this.worldObj.getBlockMetadata(tEvent.x, tEvent.y, tEvent.z);
						if (!this.worldObj.isAirBlock(tEvent.x, tEvent.y, tEvent.z) && !this.isBlockGlass(new ItemStack(tBlock, 1, meta)))
							if (!this.canMine(tEvent, tBlock))
								this.setDead();
							else if (IC2.platform.isSimulating())
							{
								...
								if (this.power >= 0.0F)
								{
									if (tBlock.getMaterial() != Material.tnt && tBlock.getMaterial() != Material.tnt)
									{
										if (this.smelt)
											if (tBlock.getMaterial() == Material.wood)
												...
												for (ItemStack isa : tBlock.getDrops(this.worldObj, tEvent.x, tEvent.y, tEvent.z, this.worldObj.getBlockMetadata(tEvent.x, tEvent.y, tEvent.z), 0))
												{
													...
													if (is != null)
													{
														Block newBlock = Block.getBlockFromItem(is.getItem());
														if (newBlock != Blocks.air && newBlock != tBlock)
														{
	...
	{
		return item.isItemEqual(Ic2Items.reinforcedGlass) ? true : StackUtil.hasOreTag("blockGlass", item) || StackUtil.hasOreTag("paneGlass", item);
	}
	...
	public void explode()
	{
		if (IC2.platform.isSimulating())
		{
			...
			if (this.takeDataFromEvent(tEvent))
			{
				...
				if (ForgeEventFactory.onExplosionStart(this.worldObj, explosion.fakeExplosion))
					return;
	...
	public BreakEvent getEventFromEvent(LaserEvent.LaserHitsBlockEvent evt)
	{
		EntityPlayer player = null;
		if (evt.owner instanceof EntityPlayer)
			player = (EntityPlayer) evt.owner;
		else
			player = FakePlayerFactory.getMinecraft((WorldServer) evt.world);

		return new BreakEvent(evt.x, evt.y, evt.z, evt.world, evt.world.getBlock(evt.x, evt.y, evt.z), evt.world.getBlockMetadata(evt.x, evt.y, evt.z), player);
	}
