...
public class HoeModule extends PowerModuleBase implements IPowerModule, IRightClickModule
{
	...
	public HoeModule(List<IModularItem> validItems)
	{
		...
		this.addBaseProperty(HOE_ENERGY_CONSUMPTION, 50);
		this.addTradeoffProperty("Search Radius", HOE_ENERGY_CONSUMPTION, 950);
		this.addTradeoffProperty("Search Radius", HOE_SEARCH_RADIUS, 8, "m");
	}
	...
	public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ)
	{
		...
		if (player.canPlayerEdit(x, y, z, side, itemStack) && ElectricItemUtils.getPlayerEnergy(player) > energyConsumed)
		{
				...
				for (int j = (int) Math.floor(-radius); j < radius; j++)
					if (i * i + j * j < radius * radius)
					{
						...
						if (side != 0 && world.getBlock(newX, y + 1, newZ).isAir(world, newX, y + 1, newZ) && (block == Blocks.grass || block == Blocks.dirt))
							...
							if (EventUtils.cantBreak(player, newX, y, newZ))
			...
			if (used)
			{
				world.playSoundEffect(x + 0.5F, y + 0.5F, z + 0.5F, Blocks.farmland.stepSound.getStepResourcePath(), (Blocks.farmland.stepSound.getVolume() + 1.0F) / 2.0F, Blocks.farmland.stepSound.getPitch() * 0.8F);
				return true;
			}
		}
		return false;
