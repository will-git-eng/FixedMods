...
public class TileEntityHarvester extends TileEntityFactoryPowered implements ITankContainerBucketable
{
	...
	public boolean activateMachine()
	{
		...
		List var5 = var4.getDrops(this.worldObj, this._rand, this._immutableSettings, var1.x, var1.y, var1.z);
		var4.preHarvest(this.worldObj, var1.x, var1.y, var1.z);
		if (var5 instanceof ArrayList)
			ForgeEventFactory.fireBlockHarvesting((ArrayList) var5, this.worldObj, var2, var1.x, var1.y, var1.z, var3, 0, 1.0F, this._settings.get("silkTouch") == Boolean.TRUE, null);

		if (var4.breakBlock())
		{
			if (!this.worldObj.setBlock(var1.x, var1.y, var1.z, Blocks.air, 0, 2))
				return false;

			if (this._settings.get("playSounds") == Boolean.TRUE)
				this.worldObj.playAuxSFXAtEntity(null, 2001, var1.x, var1.y, var1.z, Block.getIdFromBlock(var2) + (var3 << 12));
		}

		this.setIdleTicks(this.getExtraIdleTime(10));
		this.doDrop(var5);
		this._tanks[0].fill(FluidRegistry.getFluidStack("sludge", 10), true);
		var4.postHarvest(this.worldObj, var1.x, var1.y, var1.z);
		return true;
	}
