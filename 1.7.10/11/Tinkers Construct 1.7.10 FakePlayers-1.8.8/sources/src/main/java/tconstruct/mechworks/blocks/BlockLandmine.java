...
public class BlockLandmine extends BlockContainer
	...
	public void breakBlock(World par1World, int par2, int par3, int par4, Block par5Block, int par6)
	{
		...
		if (tileentity != null && (!this.explodeOnBroken || !this.hasItems(par1World, par2, par3, par4)) && par6 != 193 && !tileentity.isExploding)
		{
			if (this != null)
			{
				ItemStack is = new ItemStack(this, 1, this.damageDropped(tileentity.triggerType));
				if (tileentity.hasCustomInventoryName())
					is.setStackDisplayName(tileentity.getInventoryName());
				this.dropBlockAsItem(par1World, par2, par3, par4, new ItemStack(this, 1, this.damageDropped(tileentity.triggerType)));
			}
			...
			for (int j1 = 0; j1 < tileentity.getSizeInventory(); ++j1)
			{
				...
				if (itemstack != null)
					while (itemstack.stackSize > 0)
					{
						...
						EntityItem entityitem = new EntityItem(par1World, par2, par3, par4, new ItemStack(itemstack.getItem(), ss, itemstack.getItemDamage()));
	...
	{
		if (skipCheck || this.getMineState(par1World, par2, par3, par4) > 0)
			if (this.hasItems(par1World, par2, par3, par4))
			{
	...
	protected int getMineState(World par1World, int par2, int par3, int par4)
		...
		if (!(par1World.getTileEntity(par2, par3, par4) instanceof TileEntityLandmine))
		...
		if (triggerType != null)
		{
			...
			if (triggerType == Sensitivity.everything)
				list = par1World.getEntitiesWithinAABBExcludingEntity(null, this.getSensitiveAABB(par1World, par2, par3, par4));

			if (triggerType == Sensitivity.mobs)
				list = par1World.getEntitiesWithinAABB(EntityLivingBase.class, this.getSensitiveAABB(par1World, par2, par3, par4));
	...
	public Entity getMineTriggerer(World par1World, int par2, int par3, int par4)
	{
		...
		if (triggerType != null)
		{
			...
			if (triggerType == Sensitivity.everything)
				list = par1World.getEntitiesWithinAABBExcludingEntity(null, AxisAlignedBB.getBoundingBox(par2 + 0D, par3 + 0D, par4 + 0D, par2 + 1D, par3 + 1D, par4 + 1D));
