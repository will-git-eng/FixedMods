...
public class ContainerEnder<T extends IInventory> extends Container
{
	...
	{
		if (!this.needClose && this.inv instanceof TileEntityEnder)
			((TileEntityEnder) this.inv).isGuiOpened = false;
	...
	public ContainerEnder(InventoryPlayer playerInv, T inv)
	{
		...
		{
			TileEntityEnder tile = (TileEntityEnder) inv;
			this.needClose = tile.isGuiOpened;
			if (!this.needClose)
	...
	protected boolean mergeItemStack(ItemStack par1ItemStack, int fromIndex, int toIndex, boolean reversOrder)
	{
		...
		if (par1ItemStack.isStackable())
		{
			...
			while (par1ItemStack.stackSize > 0 && (!reversOrder && checkIndex < toIndex || reversOrder && checkIndex >= fromIndex))
			{
				...
				if (itemstack1 != null && itemstack1.getItem() == par1ItemStack.getItem() && (!par1ItemStack.getHasSubtypes() || par1ItemStack.getItemDamage() == itemstack1.getItemDamage()) && ItemStack.areItemStackTagsEqual(par1ItemStack, itemstack1) && slot.isItemValid(par1ItemStack) && par1ItemStack != itemstack1)
				{
