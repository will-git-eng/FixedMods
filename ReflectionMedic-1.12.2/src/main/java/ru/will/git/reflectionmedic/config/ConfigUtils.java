...
public final class ConfigUtils
{
	...
	{
		Configuration cfg = new Configuration(new File(reflectionmedicMod.CFG_DIR, cfgName + ".cfg"));
		cfg.load();
		return cfg;
	}
	...
	public static void readConfig(@Nonnull Class<?> configClass, @Nonnull String configName, boolean reload)
	{
		...
		try
		{
			for (Field field : configClass.getDeclaredFields())
			{
				...
				{
					reflectionmedicMod.LOGGER.error("Failed reading property {} in config {}", field, cfg.getConfigFile().getName(), throwable);
				}
			}
		}
		catch (Throwable throwable)
		{
			reflectionmedicMod.LOGGER.error("Failed reading config {}", cfg.getConfigFile().getName(), throwable);
		}
		cfg.save();
	}
